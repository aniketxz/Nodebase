...
const StatusBorder = ({
	...
	return (
		<>
			<div
				className={cn(
					"absolute -top-[1.5px] -left-[1.5px] h-[calc(100%+3px)] w-[calc(100%+3px)] rounded-md border-4",
					className,
				)}
			/>
			{children}
...
export const NodeStatusIndicator = ({
	...
	switch (status) {
		...
		case "success":
			return (
				<StatusBorder className={cn("border-green-700/60", className)}>
					{children}
				</StatusBorder>
			);
		case "error":
			return (
				<StatusBorder className={cn("border-red-500/70", className)}>
					{children}
				</StatusBorder>
			);
		default:
